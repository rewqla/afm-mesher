...
from src.application.services.mesh_postprocessing import renumber_nodes_multilevel_rcm, renumber_nodes_rcm_multistart
...
def main() -> None:
    ...
    leaf_sizes = args.leaf_size or [50, 75, 150]
    ...
    for case in cases:
        ...
        for leaf_size in leaf_sizes:
            ...
            multilevel_times: list[float] = []
            baseline_beta = None
            multilevel_beta = None

            for _ in range(args.runs):
                t0 = time.perf_counter()
                _, _, baseline_beta = renumber_nodes_rcm_multistart(nodes, triangles)
                t1 = time.perf_counter()
                baseline_times.append((t1 - t0) * 1000.0)

                t0 = time.perf_counter()
                _, _, multilevel_beta = renumber_nodes_multilevel_rcm(nodes, triangles, leaf_size=leaf_size)
                t1 = time.perf_counter()
                multilevel_times.append((t1 - t0) * 1000.0)
            ...
            row = {
                ...
                "multilevel_beta": multilevel_beta,
                "delta_beta": (baseline_beta - multilevel_beta) if baseline_beta is not None and multilevel_beta is not None else None,
                "baseline_time_ms": statistics.median(baseline_times),
                "multilevel_time_ms": statistics.median(multilevel_times),
                "time_overhead_ms": statistics.median(multilevel_times) - statistics.median(baseline_times),
            }
            rows.append(row)
            print(
                f"{case_name}: leaf={leaf_size} nodes={row['nodes']} triangles={row['triangles']} "
                f"baseline_beta={row['baseline_beta']} multilevel_beta={row['multilevel_beta']} "
                f"baseline_ms={row['baseline_time_ms']:.3f} multilevel_ms={row['multilevel_time_ms']:.3f}"
            )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Profile the experimental multilevel RCM strategy.")
    parser.add_argument("--image", action="append", default=[], help="Image path to reconstruct a raw AFM graph before RCM.")
    parser.add_argument("--grid-size", action="append", type=int, default=[], help="Synthetic rectangular grid size in nodes.")
    parser.add_argument("--leaf-size", action="append", type=int, default=[], help="Leaf size for recursive bisection.")
    parser.add_argument("--runs", type=int, default=1, help="Number of measurements to aggregate per case.")
    parser.add_argument("--output-dir", type=Path, default=Path("data") / "output", help="Directory for CSV/JSON reports.")
...
def _write_reports(output_dir: Path, rows: list[dict[str, object]]) -> None:
    ...
    json_path = output_dir / "multilevel_rcm_report.json"
    csv_path = output_dir / "multilevel_rcm_report.csv"
